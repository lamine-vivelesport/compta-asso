...
function guessMapping(headers: string[]): Record<EcritureField, string> {
  ...
  const candidates: Record<EcritureField, string[]> = {
    ...
    compte_debit: ['comptedebit', 'debit', 'comptedeb', 'deb'],
    compte_credit: ['comptecredit', 'credit', 'comptecre', 'cre'],
    montant: ['montant', 'amount', 'valeur', 'somme'],
  }

  for (const field of ECRITURE_FIELDS) {
    for (const h of headers) {
      if (candidates[field].includes(normalize(h))) {
        mapping[field] = h
        break
      }
    }
...
export default function ImportPage() {
  ...
  const handleImport = async () => {
    ...
    if (valid.length === 0) {
      showToast('error', 'Aucune ligne valide à importer. Vérifiez le mapping des colonnes.')
      return
    }
    ...
    if (errors === 0) {
      showToast('success', `${success} écriture(s) importée(s) avec succès.`)
    } else {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      >
        ...
        {rows.length > 0 && (
          <p className="mt-3 text-indigo-600 text-sm font-medium">{rows.length} ligne(s) chargée(s)</p>
        )}
        <input
          ref={inputRef}
          type="file"
        ...
        <>
          ...
          {result && (
            <div className={`mb-4 p-4 rounded-xl border ${result.errors === 0 ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'}`}>
              <p className="font-semibold text-gray-800">Résultat de l&apos;import :</p>
              <p className="text-sm text-green-700">{result.success} écriture(s) importée(s) avec succès</p>
          ...
          >
            {importing ? 'Import en cours...' : `Importer ${rows.length} écriture(s)`}
          </button>
        </>
      )}
    </div>
